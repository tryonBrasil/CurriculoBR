...
export default function App() {
  ...
  const handlePrint = () => {
    try {
      // Captura o elemento do currículo JÁ renderizado em tamanho 1:1 (794×1123px)
      ...
      // Clona preservando TODOS os estilos inline (incluindo font-size no root)
      const clone = el.cloneNode(true) as HTMLElement;

      // Remove interatividade
      clone.querySelectorAll('[draggable]').forEach(n => n.removeAttribute('draggable'));
      clone.querySelectorAll('.no-print').forEach(n => (n as HTMLElement).style.display = 'none');

      // Força dimensões A4 + fonte sem apagar as classes Tailwind do template
      ...
      // Coleta todos os estilos da página (Tailwind, fontes, etc.)
...
<body>
  ...
  <script>
    window.onload = function () {
      // Aguarda fontes e imagens (Google Fonts pode demorar)
      if (document.fonts && document.fonts.ready) {
        document.fonts.ready.then(function () {
          setTimeout(function () { window.print(); setTimeout(window.close, 800); }, 400);
        });
      } else {
        setTimeout(function () { window.print(); setTimeout(window.close, 800); }, 1200);
      }
    };
  <\/script>
</body>
      ...
      if (!w) {
        showToast('Pop-up bloqueado. Permita pop-ups para baixar o PDF.', 'error');
        return;
      }
      w.document.open();
  ...
  const fitToScreen = useCallback(() => {
    ...
    // Desconta toolbar h-12 (48px) + padding vertical (64px)
    const ch = previewContainerRef.current.clientHeight - 48 - 64;
    const cw = previewContainerRef.current.clientWidth - 48;
    const scaleH = ch / 1123;
    const scaleW = cw / 794;
    setPreviewScale(Math.min(0.95, Math.max(0.25, Math.min(scaleH, scaleW))));
  }, []);

  useEffect(() => {
    if (view === 'editor') {
      // 350ms: aguarda a transição CSS de abertura/fechamento da sidebar (duration-300)
          ...
          <div className="bg-slate-900 w-full max-w-sm rounded-2xl shadow-2xl border border-slate-700 p-6 animate-in zoom-in-95 duration-200">
            <div className="flex items-center gap-3 mb-5">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500">CurriculoBR · Admin</p>
              </div>
            ...
            <p className="text-center text-xs text-slate-700 mt-4 uppercase tracking-widest">
        ...
        <div className="fixed inset-0 z-[500] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
          <div className="bg-white dark:bg-slate-900 w-full max-w-2xl rounded-3xl shadow-2xl p-8 border border-slate-100 dark:border-slate-800 animate-in zoom-in-95 duration-200">
            ...
            <div className="flex gap-4">
              <button onClick={() => setIsImportModalOpen(false)} className="flex-1 py-4 font-bold text-slate-500 uppercase text-xs tracking-widest">Cancelar</button>
              <button 
      ...
      <div className={`min-h-screen flex flex-col transition-colors duration-300 ${isDarkMode ? 'dark bg-slate-900' : 'bg-slate-50'}`}>
        ...
        <header className="h-16 bg-white dark:bg-slate-950 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between px-6 sticky top-0 z-50 shadow-sm">
          ...
          <div className="flex items-center gap-1 bg-slate-100 dark:bg-slate-800 rounded-full p-1">
            <button onClick={() => navigateTo('/', 'editor')} className="px-3 py-2 text-xs font-bold uppercase text-slate-500 dark:text-slate-400 hover:text-blue-600 transition-colors rounded-full">Currículo</button>
            <span className="px-3 py-2 text-xs font-bold uppercase text-white bg-blue-600 rounded-full">Carta</span>
          </div>
        ...
        <main className="flex-1 max-w-6xl mx-auto w-full px-4 md:px-8 py-8 md:py-12">
          ...
          <div className="text-center mb-10">
            <span className="inline-block px-4 py-2 bg-blue-50 dark:bg-slate-800 text-blue-600 dark:text-blue-400 rounded-full text-xs font-black uppercase tracking-[0.2em] mb-4">
            ...
            <div className="space-y-5">
              <div className="bg-white dark:bg-slate-800 rounded-3xl p-6 border border-slate-100 dark:border-slate-700 shadow-sm">
                ...
                <div className="space-y-4">
                  <div>
                    <label className="block text-xs font-black text-slate-400 uppercase tracking-widest mb-1.5">Cargo / Vaga</label>
                    <input
                  ...
                  <div>
                    <label className="block text-xs font-black text-slate-400 uppercase tracking-widest mb-1.5">Empresa</label>
                    <input
                      className="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-600 text-sm bg-slate-50 dark:bg-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-500 transition-all"
                  ...
                  <div>
                    <label className="block text-xs font-black text-slate-400 uppercase tracking-widest mb-1.5">Seu Nome (para a carta)</label>
                    <input
                      className="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-600 text-sm bg-slate-50 dark:bg-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-500 transition-all"
                ...
                <div className="grid grid-cols-3 gap-3">
                  {tones.map(t => (
                    ...
                    >
                      ...
                      <span className={`text-xs font-black uppercase tracking-wide ${clTone === t.id ? 'text-blue-700 dark:text-blue-400' : 'text-slate-500 dark:text-slate-400'}`}>{t.label}</span>
              ...
              <div className="bg-white dark:bg-slate-800 rounded-3xl p-6 border border-slate-100 dark:border-slate-700 shadow-sm">
                <h2 className="text-xs font-black text-slate-400 uppercase tracking-widest mb-3 flex items-center gap-2">
                  ...
                  <span className="ml-auto text-xs font-bold text-slate-300 normal-case tracking-normal">Opcional</span>
                </h2>
                <textarea
      ...
      <div className="min-h-screen bg-white dark:bg-slate-900 flex flex-col relative overflow-hidden transition-colors duration-300">
        ...
        <main className="relative z-10 flex-1 flex flex-col items-center justify-center px-6 pt-12 pb-24 text-center">
          <div className="max-w-5xl w-full space-y-8">
            <div className="space-y-4 animate-in fade-in slide-in-from-top-4 duration-1000">
              <span className="inline-flex items-center gap-2 py-2 px-4 bg-blue-50 dark:bg-slate-800 text-blue-600 dark:text-blue-400 rounded-full text-xs font-black uppercase tracking-[0.2em] mb-4">
            ...
            <div className="mt-4">
               <button onClick={() => navigateTo('/carta-de-apresentacao', 'cover-letter-page')} className="text-slate-400 hover:text-blue-600 text-xs font-bold uppercase tracking-widest transition-colors flex items-center justify-center gap-2 mx-auto group">
                 ✉️ Precisa de uma Carta de Apresentação também? <i className="fas fa-arrow-right text-xs group-hover:translate-x-1 transition-transform"></i>
               </button>
            </div>
        ...
        <section className="relative z-10 py-16 px-6 bg-slate-50 dark:bg-slate-900 border-t border-slate-100 dark:border-slate-800">
          <div className="max-w-5xl mx-auto">
            <p className="text-center text-xs font-black uppercase tracking-[0.25em] text-blue-600 dark:text-blue-400 mb-12">Por que o CurriculoBR? 🤔</p>
        ...
        <section className="relative z-10 py-16 px-6 bg-white dark:bg-slate-950 border-t border-slate-100 dark:border-slate-800 overflow-hidden">
          ...
          <div className="max-w-5xl mx-auto relative z-10">
            <div className="grid md:grid-cols-2 gap-12 items-center">
              <div>
                <span className="inline-block px-3 py-1 bg-violet-100 dark:bg-violet-900/30 text-violet-600 dark:text-violet-400 rounded-full text-xs font-black uppercase tracking-[0.2em] mb-4">Exclusivo</span>
              ...
              <div className="bg-gradient-to-br from-violet-600 to-purple-700 rounded-3xl p-6 shadow-2xl">
                <div className="bg-white/10 rounded-2xl p-5 backdrop-blur-sm">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <span className="bg-green-400 text-green-900 text-xs font-black px-2 py-0.5 rounded-full uppercase">Excelente</span>
                  </div>
                  <div className="flex items-center gap-5 mb-5">
                    ...
                    <div>
                      <p className="text-white/60 text-xs uppercase tracking-widest font-bold mb-2">Palavras-chave</p>
                      <div className="flex flex-wrap gap-1">
                        {['React', 'TypeScript', 'Liderança', 'Agile'].map(kw => (
                          <span key={kw} className="bg-white/20 text-white text-xs font-bold px-2 py-0.5 rounded-full">{kw}</span>
                        ))}
                      </div>
                    </div>
                  </div>
                  <div className="space-y-2">
                    <p className="text-xs font-black text-white/50 uppercase tracking-widest">Melhorias</p>
        ...
        <section className="relative z-10 py-16 px-6 bg-slate-50 dark:bg-slate-900 border-t border-slate-100 dark:border-slate-800">
          <div className="max-w-5xl mx-auto">
            <p className="text-center text-xs font-black uppercase tracking-[0.25em] text-blue-600 dark:text-blue-400 mb-3">Depoimentos</p>
        ...
        <section className="relative z-10 py-16 px-6 bg-white dark:bg-slate-950 border-t border-slate-100 dark:border-slate-800">
          <div className="max-w-5xl mx-auto">
            <div className="flex items-center justify-between mb-8">
              <div>
                <span className="text-xs font-black uppercase tracking-widest text-blue-600 dark:text-blue-400">Blog</span>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {[
                ...
                >
                  <span className="text-xs font-black uppercase tracking-widest text-blue-600 dark:text-blue-400 mb-2 block">{post.cat}</span>
                  ...
                  <p className="text-xs text-blue-600 dark:text-blue-400 font-bold uppercase mt-4">Ler artigo →</p>
                </div>
              ))}
            </div>
        ...
        <footer className="relative z-10 py-8 bg-slate-50 dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800 text-center">
          ...
          <p className="text-xs text-slate-400 dark:text-slate-600">© 2026 CurriculoBR. Todos os direitos reservados.</p>
        </footer>
      </div>
    );
  }
      ...
      <div className="min-h-screen bg-slate-50 dark:bg-slate-900 flex flex-col transition-colors duration-300">
        ...
        <header className="h-16 bg-white dark:bg-slate-950 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between px-6 md:px-8 sticky top-0 z-50">
          ...
          <div className="text-center">
            ...
            <p className="text-xs text-slate-400 font-medium hidden sm:block">Qual vai ser o look do seu próximo emprego?</p>
          </div>
          <div className="w-20"></div>
        </header>
              ...
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-400 font-bold">{FREE_TEMPLATES.length} modelos para começar agora</p>
                  </div>
                </div>
                <button onClick={() => setShowFreeTemplates(v => !v)} className="flex items-center gap-2 px-4 py-2 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 text-xs font-black uppercase tracking-wide hover:bg-slate-200 dark:hover:bg-slate-700 transition-all">
                  ...
                  {FREE_TEMPLATES.map((t) => (
                    <div key={t.id} className={`bg-white dark:bg-slate-800 rounded-3xl overflow-hidden shadow-lg hover:shadow-2xl hover:-translate-y-2 transition-all duration-300 group border-2 flex flex-col ${template === t.id ? 'border-blue-600 ring-4 ring-blue-100 dark:ring-blue-900/30' : 'border-transparent hover:border-blue-200 dark:hover:border-blue-800'}`}>
                      <div className="relative aspect-[210/297] bg-slate-100 dark:bg-slate-900 overflow-hidden">
                        ...
                        {t.badge && <div className={`absolute top-3 left-3 ${t.badgeColor} text-white text-xs font-black px-2 py-1 rounded-full`}>{t.badge}</div>}
                        {template === t.id && <div className="absolute top-3 right-3 bg-blue-600 text-white text-xs font-black px-2 py-1 rounded-full">✓ Ativo</div>}
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      {isPremium
                        ? <span className="bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 text-xs font-black px-2 py-0.5 rounded-full uppercase">✓ Ativado</span>
                        : <span className="bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 text-xs font-black px-2 py-0.5 rounded-full uppercase">R$ 9,90 único</span>
                      }
                    </div>
                    <p className="text-xs text-slate-400 font-bold">{PREMIUM_TEMPLATES_LIST.length} designs exclusivos</p>
                  </div>
                </div>
                {!isPremium && (
                  <button onClick={() => { setPremiumModalTemplate(''); setIsPremiumModalOpen(true); }} className="flex items-center gap-2 px-4 py-2 rounded-xl bg-gradient-to-r from-amber-400 to-orange-500 text-white text-xs font-black uppercase tracking-wide hover:opacity-90 transition-all shadow-md active:scale-95">
                    🔓 Desbloquear tudo
                  </button>
                )}
              </div>
                  ...
                  return (
                    <div key={t.id} className={`rounded-3xl overflow-hidden shadow-lg transition-all duration-300 group border-2 flex flex-col ${unlocked ? 'bg-white dark:bg-slate-800 hover:shadow-2xl hover:-translate-y-2' : 'bg-slate-50 dark:bg-slate-800/60'} ${template === t.id ? 'border-amber-500 ring-4 ring-amber-100 dark:ring-amber-900/30' : unlocked ? 'border-transparent hover:border-amber-300 dark:hover:border-amber-700' : 'border-slate-200 dark:border-slate-700'}`}>
                      <div className="relative aspect-[210/297] bg-slate-100 dark:bg-slate-900 overflow-hidden">
                        ...
                        {t.badge && <div className={`absolute top-3 left-3 ${t.badgeColor} text-white text-xs font-black px-2 py-1 rounded-full`}>{t.badge}</div>}
                        {template === t.id && unlocked && <div className="absolute top-3 right-3 bg-amber-500 text-white text-xs font-black px-2 py-1 rounded-full">✓ Ativo</div>}
  ...
  return (
    <div className="h-screen flex flex-col bg-white dark:bg-slate-950 overflow-hidden transition-colors duration-300 google-auto-ads-ignore">
      ...
      <nav className="no-print h-14 border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 flex items-center justify-between px-3 md:px-6 z-50 shrink-0">
        ...
        <div className="flex md:hidden bg-slate-100 dark:bg-slate-800 p-1 rounded-xl">
           <button 
             ...
             className={`px-3 py-1.5 rounded-lg text-xs font-bold uppercase tracking-wide transition-all flex items-center gap-1.5 ${mobileView === 'editor' ? 'bg-white dark:bg-slate-700 text-blue-600 shadow-sm' : 'text-slate-400'}`}
           >
             <i className="fas fa-pencil-alt text-xs"></i> Editar
           </button>
           <button 
             onClick={() => setMobileView('preview')} 
             className={`px-3 py-1.5 rounded-lg text-xs font-bold uppercase tracking-wide transition-all flex items-center gap-1.5 ${mobileView === 'preview' ? 'bg-white dark:bg-slate-700 text-blue-600 shadow-sm' : 'text-slate-400'}`}
           >
             <i className="fas fa-eye text-xs"></i> Ver
           </button>
        </div>
        ...
        <div className="hidden lg:flex items-center gap-6">
           ...
           <button
             ...
             className="flex items-center gap-2 px-4 py-2 bg-violet-50 dark:bg-violet-900/20 text-violet-700 dark:text-violet-400 border border-violet-200 dark:border-violet-700/40 rounded-full font-black text-xs font-bold uppercase tracking-wide hover:bg-violet-100 dark:hover:bg-violet-900/40 transition-all"
        ...
        <div className="flex md:hidden items-center gap-1">
          <button onClick={handlePrint} className="bg-blue-600 text-white px-3 py-2 rounded-full text-xs font-bold uppercase flex items-center gap-1.5">
      ...
      <div className="flex-1 flex overflow-hidden relative">
        ...
        <div className={`no-print w-full md:w-[440px] lg:w-[480px] flex flex-col border-r border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 z-30 shrink-0 transition-all duration-300 absolute md:relative inset-0 md:inset-auto ${mobileView === 'editor' ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
           ...
           <div className="flex overflow-x-auto border-b border-slate-50 dark:border-slate-800 shrink-0 custom-scrollbar bg-slate-50/50 dark:bg-slate-900/50 px-2">
             {STEPS.map((step, idx) => (
               ...
               >
                 ...
                 <span className="text-xs font-bold uppercase tracking-wide whitespace-nowrap">{step.label}</span>
           ...
           <div ref={editorScrollRef} className={`flex-1 overflow-y-auto custom-scrollbar p-6 md:p-8 transition-colors duration-500 ${highlightedStep ? 'bg-blue-50/20 dark:bg-blue-900/10' : ''}`}>
              {activeTab === 'info' && (
                <div className="animate-in slide-in-from-bottom-2 duration-300">
                  ...
                  <p className="text-sm text-slate-500 dark:text-slate-400 mb-5">Essas são as primeiras impressões — capriche! ✨</p>
                  ...
                  <div className="flex items-center gap-6 mb-6">
                    ...
                    <div>
                       ...
                       {data.personalInfo?.photoUrl && (
                           <button onClick={(e) => { e.stopPropagation(); updateData(p => ({...p, personalInfo: {...p.personalInfo, photoUrl: ''}})); }} className="text-xs text-red-500 font-bold uppercase mt-2 hover:underline">Remover foto</button>
                       )}
                    </div>
                  </div>
              ...
              {activeTab === 'experience' && (
                <div className="animate-in slide-in-from-bottom-2 duration-300">
                  <div className="flex justify-between items-center mb-2">
                    ...
                    <button onClick={() => addItem('experiences')} className="bg-blue-600 text-white px-4 py-2 rounded-xl font-bold text-sm shadow-sm hover:bg-blue-700 active:scale-95 transition-all">+ Adicionar</button>
                  </div>
                  <p className="text-sm text-slate-500 dark:text-slate-400 mb-5">Seus maiores feitos vão aqui. Não seja modesto! 💪</p>
                    ...
                    <div key={exp.id} className="p-5 bg-slate-50 dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 mb-6 relative group border-l-4 border-l-blue-400 shadow-sm hover:shadow-md transition-shadow">
                      ...
                      <div className="mt-2 relative">
                         <div className="flex justify-between items-center mb-1">
                           <label className="text-sm font-semibold text-slate-500 dark:text-slate-400">Descrição</label>
                           <button onClick={() => handleEnhance(exp.description, 'experiência', 'experiences', exp.id)} disabled={!exp.description || isEnhancing === exp.id} className="text-xs text-blue-600 dark:text-blue-400 font-bold uppercase hover:text-blue-800 transition-colors flex items-center gap-1.5">
              ...
              {activeTab === 'education' && (
                <div className="animate-in slide-in-from-bottom-2 duration-300">
                  <div className="flex justify-between items-center mb-2">
                    ...
                    <button onClick={() => addItem('education')} className="bg-blue-600 text-white px-4 py-2 rounded-xl font-bold text-sm shadow-sm hover:bg-blue-700 active:scale-95 transition-all">+ Adicionar</button>
                  </div>
                  <p className="text-sm text-slate-500 dark:text-slate-400 mb-5">Onde você aprendeu a ser incrível! 🏫</p>
              ...
              {activeTab === 'languages' && (
                <div className="animate-in slide-in-from-bottom-2 duration-300">
                  <div className="flex justify-between items-center mb-2">
                    ...
                    <button onClick={() => addItem('languages')} className="bg-blue-600 text-white px-4 py-2 rounded-xl font-bold text-sm shadow-sm hover:bg-blue-700 active:scale-95 transition-all">+ Adicionar</button>
                  </div>
                  <p className="text-sm text-slate-500 dark:text-slate-400 mb-5">Fala inglês? Ótimo. Fala mandarim? Melhor ainda. 😏</p>
              ...
              {activeTab === 'certifications' && (
                <div className="animate-in slide-in-from-bottom-2 duration-300">
                  <div className="flex justify-between items-center mb-2">
                    ...
                    <button onClick={() => addItem('courses')} className="bg-blue-600 text-white px-4 py-2 rounded-xl font-bold text-sm shadow-sm hover:bg-blue-700 active:scale-95 transition-all">+ Adicionar</button>
                  </div>
                  <p className="text-sm text-slate-500 dark:text-slate-400 mb-5">Cursos, certificações e tudo que você aprendeu além da faculdade. 📚</p>
              ...
              {activeTab === 'projects' && (
                <div className="animate-in slide-in-from-bottom-2 duration-300">
                  <div className="flex justify-between items-center mb-2">
                    ...
                    <button onClick={() => addItem('projects')} className="bg-blue-600 text-white px-4 py-2 rounded-xl font-bold text-sm shadow-sm hover:bg-blue-700 active:scale-95 transition-all">+ Adicionar</button>
                  </div>
                  <p className="text-sm text-slate-500 dark:text-slate-400 mb-5">Mostre o que você construiu! Projetos pessoais, open source, apps, sites... 🔨</p>
                ...
                <div className="animate-in slide-in-from-bottom-2 duration-300">
                  <div className="flex justify-between items-center mb-2">
                    <h2 className="text-xl font-black text-slate-900 dark:text-white uppercase tracking-tight">⚡ Skills</h2>
                     <button onClick={handleSuggestSkills} disabled={isEnhancing === 'skills-suggest'} className="text-sm font-bold text-blue-600 dark:text-blue-400 uppercase flex items-center gap-2 hover:text-blue-800 transition-colors">
                      <i className={`fas ${isEnhancing === 'skills-suggest' ? 'fa-circle-notch fa-spin' : 'fa-wand-magic-sparkles'}`}></i> 🤖 Sugerir com IA
                    </button>
                  </div>
                  <p className="text-sm text-slate-500 dark:text-slate-400 mb-5">Separe por vírgula. Seja honesto — ninguém quer "expert em tudo". 😅</p>
              ...
              {activeTab === 'summary' && (
                <div className="animate-in slide-in-from-bottom-2 duration-300">
                  <div className="flex justify-between items-center mb-2">
                    ...
                    <button onClick={handleGenerateSummary} disabled={!data.skills?.length || isEnhancing === 'summary-gen'} className="text-sm font-bold text-blue-600 dark:text-blue-400 uppercase flex items-center gap-2 hover:text-blue-800 transition-colors">
                      <i className={`fas ${isEnhancing === 'summary-gen' ? 'fa-circle-notch fa-spin' : 'fa-wand-magic'}`}></i> 🤖 Gerar com IA
                    </button>
                  </div>
                  <p className="text-sm text-slate-500 dark:text-slate-400 mb-5">Seu pitch de 3 linhas para o recrutador. A IA faz por você! 🚀</p>
                  <div className="relative">
                    <textarea
                    ...
                    <button
                      ...
                      className="absolute bottom-4 right-4 bg-white/80 dark:bg-slate-900/80 backdrop-blur px-4 py-2 rounded-xl text-xs font-black text-slate-600 dark:text-slate-300 shadow-sm border border-slate-100 dark:border-slate-700 hover:text-blue-600 transition-all active:scale-95"
                    >
           ...
           <div className="p-4 md:p-5 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between gap-3 shrink-0 no-print bg-white dark:bg-slate-900 z-10">
              ...
              <div className="flex md:hidden items-center gap-1.5 min-w-0">
                ...
                <span className="text-xs font-black text-slate-500 dark:text-slate-400">{cvScore}% {cvScore > 70 ? '🔥' : '📝'}</span>
              </div>
              <button onClick={prevStep} className={`hidden md:block flex-1 py-4 font-bold text-xs uppercase tracking-widest text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300 transition-colors ${currentStep === 0 ? 'invisible' : ''}`}>← Anterior</button>
        ...
        <div
          ...
          className={`flex-1 overflow-hidden flex flex-col transition-all duration-300 ${mobileView === 'preview' ? 'flex' : 'hidden md:flex'}`}
        >
          {/* Toolbar da preview */}
          <div className="no-print h-12 shrink-0 flex items-center justify-between px-5 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 gap-3">
            {/* Esquerda: nome do template ativo */}
            <span className="text-sm font-bold text-slate-500 dark:text-slate-400 hidden sm:block truncate">
              {FREE_TEMPLATES.find(t => t.id === template)?.label
                || PREMIUM_TEMPLATES_LIST.find(t => t.id === template)?.label
                || template.replace(/_/g,' ')}
            </span>
            <span className="text-sm font-bold text-slate-500 dark:text-slate-400 sm:hidden truncate">
              {template.replace(/_/g,' ')}
            </span>

            {/* Centro: zoom */}
            <div className="flex items-center gap-1 shrink-0">
              <button onClick={() => setPreviewScale(s => Math.max(0.2, s - 0.05))} className="w-8 h-8 rounded-lg flex items-center justify-center text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors" title="Zoom −">
                <i className="fas fa-minus text-xs"></i>
              </button>
              <span className="text-sm font-black text-slate-700 dark:text-slate-200 w-12 text-center tabular-nums">{Math.round(previewScale * 100)}%</span>
              <button onClick={() => setPreviewScale(s => Math.min(1.0, s + 0.05))} className="w-8 h-8 rounded-lg flex items-center justify-center text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors" title="Zoom +">
                <i className="fas fa-plus text-xs"></i>
              </button>
              <button onClick={fitToScreen} className="w-8 h-8 rounded-lg flex items-center justify-center text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors ml-1" title="Ajustar à janela">
                <i className="fas fa-expand text-xs"></i>
              </button>
            </div>

            {/* Direita: toggle estilo (desktop) + editar (mobile) */}
            <div className="flex items-center gap-2 shrink-0">
              <button
                onClick={() => setMobileView('editor')}
                className="md:hidden flex items-center gap-1 text-sm font-bold text-blue-600"
              >
                <i className="fas fa-pencil-alt text-xs"></i> Editar
              </button>
              <button
                onClick={() => setIsSidebarOpen(v => !v)}
                className={`hidden md:flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm font-bold transition-all ${
                  isSidebarOpen
                    ? 'bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 border border-blue-200 dark:border-blue-800'
                    : 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700'
                }`}
                title={isSidebarOpen ? 'Ocultar estilos' : 'Mostrar estilos'}
              >
                <i className="fas fa-palette text-xs"></i>
                <span>{isSidebarOpen ? 'Ocultar' : 'Estilos'}</span>
              </button>
            </div>
          </div>
          ...
          <div
            ...
            style={{
              background: isDarkMode
                ...
                : 'linear-gradient(135deg,#d1d9e6 0%,#bec8d8 100%)',
              padding: '32px 24px 56px',
            }}
          >
            {/* Papel A4 escalado — sombra realista */}
            <div
              ...
              style={{
                ...
                boxShadow: isDarkMode
                  ? '0 20px 60px rgba(0,0,0,.85), 0 4px 12px rgba(0,0,0,.6), 0 0 0 1px rgba(255,255,255,.07)'
                  : '0 20px 60px rgba(0,0,0,.30), 0 4px 12px rgba(0,0,0,.15), 0 0 0 1px rgba(0,0,0,.07)',
                borderRadius: 2,
                overflow: 'hidden',
              }}
            >
              {/* O currículo real em escala 1:1, depois escalado via transform */}
              <div
                style={{
        ...
        {/* Aba para reabrir sidebar quando fechada (desktop) */}
        {!isSidebarOpen && (
          <button
            onClick={() => setIsSidebarOpen(true)}
            className="hidden lg:flex no-print flex-col items-center justify-center gap-1.5 px-1.5 py-5 bg-white dark:bg-slate-900 border-l border-slate-200 dark:border-slate-800 text-slate-400 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-all shrink-0 shadow-inner"
            title="Mostrar painel de estilos"
          >
            <i className="fas fa-palette text-sm"></i>
            <span style={{ writingMode: 'vertical-lr', transform: 'rotate(180deg)' }} className="text-xs font-bold uppercase tracking-widest leading-none">Estilos</span>
            <i className="fas fa-chevron-left text-xs opacity-40"></i>
          </button>
        )}
        <div className={`no-print border-l border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 flex flex-col shrink-0 z-40 transition-all duration-300 ease-in-out shadow-2xl overflow-hidden fixed inset-y-0 right-0 lg:static ${isSidebarOpen ? 'w-[340px] translate-x-0' : 'w-0 lg:w-0 translate-x-full lg:translate-x-0'}`}>
           <div className="px-5 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between h-12 shrink-0">
              <h2 className="text-sm font-black text-slate-700 dark:text-white uppercase tracking-wide flex items-center gap-2">
                <i className="fas fa-palette text-blue-500"></i> Estilos
              </h2>
              <button onClick={() => setIsSidebarOpen(false)} className="w-8 h-8 flex items-center justify-center rounded-lg text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800 transition-all"><i className="fas fa-times text-sm"></i></button>
           </div>
           <div className="flex-1 overflow-y-auto custom-scrollbar px-5 py-5 space-y-7">
              <section>
                 <div className="flex items-center justify-between mb-3">
                    <h3 className="text-sm font-bold text-slate-600 dark:text-slate-300 uppercase tracking-wide">Tamanho da Fonte</h3>
                    <div className="flex items-center gap-2">
                      <button onClick={() => setFontSize(s => Math.max(8, +(s - 0.5).toFixed(1)))} className="w-7 h-7 flex items-center justify-center rounded-md bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700 text-base font-bold transition-colors">−</button>
                      <span className="text-sm font-black text-blue-600 dark:text-blue-400 w-10 text-center tabular-nums">{fontSize}px</span>
                      <button onClick={() => setFontSize(s => Math.min(16, +(s + 0.5).toFixed(1)))} className="w-7 h-7 flex items-center justify-center rounded-md bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700 text-base font-bold transition-colors">+</button>
                    </div>
                 </div>
                 <input type="range" min="8" max="16" step="0.5" value={fontSize} onChange={(e) => setFontSize(parseFloat(e.target.value))} className="w-full h-2 bg-slate-100 dark:bg-slate-700 rounded-full appearance-none cursor-pointer accent-blue-600 mb-1.5" />
                 <div className="flex justify-between text-xs text-slate-400 font-medium">
                   <span>Menor</span><span>Padrão (12px)</span><span>Maior</span>
                 </div>
              </section>

              <section>
                <div className="flex items-center justify-between mb-3">
                  <h3 className="text-sm font-bold text-slate-600 dark:text-slate-300 uppercase tracking-wide">Fonte</h3>
                  <span className="text-sm font-bold text-blue-600 dark:text-blue-400" style={{ fontFamily }}>
                ...
                <div className="mb-3 p-3 bg-slate-50 dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700">
                  <p className="text-base font-bold text-slate-800 dark:text-slate-100" style={{ fontFamily }}>Analista de Marketing Sênior</p>
                  <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5" style={{ fontFamily }}>experiência · habilidades · educação</p>
                </div>
                <div className="grid grid-cols-2 gap-2">
                  {FONTS.map(f => (
                    <button
                      ...
                      className={`px-3 py-2.5 rounded-xl text-sm border-2 transition-all text-left truncate ${fontFamily === f.family ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-300' : 'border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400 hover:border-slate-300 dark:hover:border-slate-600'}`}
              ...
              <section>
                ...
                <div className="flex items-center justify-between mb-3">
                  ...
                  <button onClick={() => setShowFreeTemplates(v => !v)} className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-sm font-semibold text-slate-500 hover:text-blue-600 bg-slate-100 dark:bg-slate-800 hover:bg-blue-50 dark:hover:bg-blue-900/20 transition-all">
                    <i className={`fas ${showFreeTemplates ? 'fa-eye-slash' : 'fa-eye'} text-xs`}></i>
                    {showFreeTemplates ? 'Ocultar' : 'Mostrar'}
                  </button>
                </div>
                {showFreeTemplates && (
                  <div className="space-y-2">
                    {FREE_TEMPLATES.map(t => (
                      <button key={t.id} onClick={() => handleTemplateSelect(t.id as TemplateId)}
                        className={`w-full p-3 rounded-xl border-2 transition-all flex items-center gap-3 ${template === t.id ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/20 shadow-sm' : 'border-slate-100 dark:border-slate-800 hover:border-slate-300 dark:hover:border-slate-600'}`}>
                        <TemplateThumbnail template={t.id as TemplateId} className="w-14 h-[74px] shrink-0 rounded overflow-hidden" />
                        <div className="text-left flex-1 min-w-0">
                          ...
                          <p className="text-xs text-slate-500 dark:text-slate-400 truncate mt-0.5">{t.desc}</p>
                          {t.badge && <span className={`inline-block mt-1 text-xs font-bold text-white px-2 py-0.5 rounded-full ${t.badgeColor}`}>{t.badge}</span>}
                        </div>
                        {template === t.id && <div className="w-6 h-6 bg-blue-600 rounded-full flex items-center justify-center shrink-0"><i className="fas fa-check text-white text-xs"></i></div>}
                      </button>
                    ))}
                  </div>
                )}
                ...
                <div className="mt-5">
                  <div className="flex items-center gap-2 mb-3">
                    <div className="flex-1 h-px bg-gradient-to-r from-amber-300 to-orange-400 opacity-60"></div>
                    <button onClick={() => setShowPremiumTemplates(v => !v)}
                      className="flex items-center gap-1.5 text-sm font-black text-amber-600 dark:text-amber-400 hover:opacity-70 transition-opacity">
                      👑 Premium <i className={`fas fa-chevron-${showPremiumTemplates ? 'up' : 'down'} text-xs`}></i>
                    </button>
                    <div className="flex-1 h-px bg-gradient-to-l from-amber-300 to-orange-400 opacity-60"></div>
                  </div>
                  {showPremiumTemplates && (
                    <div className="space-y-2">
                      {PREMIUM_TEMPLATES_LIST.map(t => {
                        ...
                        return (
                          <button key={t.id} onClick={() => handleTemplateSelect(t.id as TemplateId)}
                            className={`w-full p-3 rounded-xl border-2 transition-all flex items-center gap-3 ${isActive && unlocked ? 'border-amber-500 bg-amber-50 dark:bg-amber-900/20 shadow-sm' : 'border-slate-100 dark:border-slate-800 hover:border-amber-300 dark:hover:border-amber-700'}`}
                          >
                            <div className="relative w-14 h-[74px] shrink-0 rounded overflow-hidden">
                              <TemplateThumbnail template={t.id as TemplateId} className={`w-full h-full ${!unlocked ? 'opacity-50 blur-[1px]' : ''}`} />
                              {!unlocked && <div className="absolute inset-0 flex items-center justify-center bg-black/10"><span className="text-xl">🔒</span></div>}
                            </div>
                            <div className="text-left flex-1 min-w-0">
                              ...
                              <p className="text-xs text-slate-500 dark:text-slate-400 truncate mt-0.5">{unlocked ? t.desc : 'Premium 👑'}</p>
                            </div>
                            {isActive && unlocked && <div className="w-6 h-6 bg-amber-500 rounded-full flex items-center justify-center shrink-0"><i className="fas fa-check text-white text-xs"></i></div>}
                          </button>
                        );
                      })}
                    </div>
                  ...
                  {!isPremium && (
                    <button onClick={() => { setPremiumModalTemplate(''); setIsPremiumModalOpen(true); }} className="mt-4 w-full py-3.5 bg-gradient-to-r from-amber-400 to-orange-500 text-white rounded-xl font-black text-sm uppercase tracking-wide hover:opacity-90 active:scale-95 transition-all shadow-md flex items-center justify-center gap-2">
                      🔓 Desbloquear — R$ 9,90
                    </button>
                  )}
                </div>
              ...
              <div className="pt-4">
                <button onClick={handleClearData} className="w-full py-3.5 bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 rounded-xl text-sm font-bold uppercase tracking-wide hover:bg-red-100 dark:hover:bg-red-900/40 transition-all border border-red-100 dark:border-red-900/20">Limpar Dados</button>
              </div>
           </div>
        </div>
      </div>
