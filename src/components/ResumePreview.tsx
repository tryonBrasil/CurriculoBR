...
const ResumePreview: React.FC<Props> = ({ data, template, onSectionClick, onReorder, fontSize = 12 }) => {
  ...
  const getTemplateLayout = () => {
    switch(template) {
      ...
      case 'english_pro':
        return (
          <div className="h-full bg-white flex flex-col overflow-hidden" style={{ fontFamily: "'Georgia', 'Times New Roman', serif" }}>
            {/* Header — name + contact bar */}
            <header className="px-10 pt-8 pb-5 cursor-pointer" onClick={() => onSectionClick?.('info')}>
              <div className="flex items-start gap-6">
                {personalInfo.photoUrl && (
                  <div className="w-20 h-20 rounded-full overflow-hidden shrink-0 border-2 border-slate-200">
                    <img src={personalInfo.photoUrl} alt="" className="w-full h-full object-cover" />
                  </div>
                )}
                <div className="flex-1">
                  <h1 className="font-black text-slate-900 leading-none mb-1" style={{ fontSize: '2em', fontFamily: 'inherit' }}>
                    {personalInfo.fullName || 'Your Name'}
                  </h1>
                  {personalInfo.jobTitle && (
                    <p className="text-[0.95em] font-semibold text-slate-500 mb-3" style={{ fontFamily: 'inherit' }}>
                      {personalInfo.jobTitle}
                    </p>
                  )}
                  {/* Contact line */}
                  <div className="flex flex-wrap gap-x-4 gap-y-1 text-[0.72em] text-slate-600" style={{ fontFamily: "'Helvetica Neue', Arial, sans-serif" }}>
                    {personalInfo.email    && <span>✉ {personalInfo.email}</span>}
                    {personalInfo.phone    && <span>📞 {personalInfo.phone}</span>}
                    {personalInfo.location && <span>📍 {personalInfo.location}</span>}
                    {personalInfo.linkedin && <span>🔗 {personalInfo.linkedin}</span>}
              ...
              {/* Divider */}
              <div className="mt-4 border-t-2 border-slate-900"></div>
            </header>

            {/* Body */}
            <div className="flex-1 px-10 pb-6 space-y-4 overflow-hidden">

              {/* Summary → "Professional Summary" */}
              {summary && (
                <div className={sectionWrapperStyle('summary')} onDragOver={(e) => handleDragOver(e, 'summary')} onDrop={(e) => handleDrop(e, 'summary')} onClick={() => onSectionClick?.('summary')}>
                  <DragHandle id="summary" />
                  <h2 className="text-[0.78em] font-black uppercase tracking-widest text-slate-900 mb-1.5" style={{ fontFamily: "'Helvetica Neue', Arial, sans-serif", letterSpacing: '0.18em' }}>Professional Summary</h2>
                  <div className="border-b border-slate-300 mb-2"></div>
                  <p className="text-[0.82em] text-slate-700 leading-relaxed" style={{ fontFamily: 'inherit' }}>{summary}</p>
                </div>
              )}
              ...
              {experiences.length > 0 && (
                <div className={sectionWrapperStyle('experience')} onDragOver={(e) => handleDragOver(e, 'experience')} onDrop={(e) => handleDrop(e, 'experience')} onClick={() => onSectionClick?.('experience')}>
                  <DragHandle id="experience" />
                  <h2 className="text-[0.78em] font-black uppercase tracking-widest text-slate-900 mb-1.5" style={{ fontFamily: "'Helvetica Neue', Arial, sans-serif", letterSpacing: '0.18em' }}>Work Experience</h2>
                  <div className="border-b border-slate-300 mb-3"></div>
                  <div className="space-y-4">
                    {experiences.map(exp => (
                      <div key={exp.id}>
                        <div className="flex justify-between items-baseline">
                          <h3 className="font-bold text-[0.9em] text-slate-900" style={{ fontFamily: 'inherit' }}>{exp.position}</h3>
                          <span className="text-[0.74em] text-slate-500 font-medium shrink-0 ml-2" style={{ fontFamily: "'Helvetica Neue', Arial, sans-serif" }}>{exp.startDate} – {exp.endDate}</span>
                        </div>
                        <p className="text-[0.82em] font-semibold italic text-slate-600 mb-1" style={{ fontFamily: 'inherit' }}>{exp.company}{exp.location ? ` · ${exp.location}` : ''}</p>
                        {exp.description && <p className="text-[0.8em] text-slate-700 leading-relaxed whitespace-pre-wrap" style={{ fontFamily: 'inherit' }}>{exp.description}</p>}
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {education.length > 0 && (
                <div className={sectionWrapperStyle('education')} onDragOver={(e) => handleDragOver(e, 'education')} onDrop={(e) => handleDrop(e, 'education')} onClick={() => onSectionClick?.('education')}>
                  <DragHandle id="education" />
                  <h2 className="text-[0.78em] font-black uppercase tracking-widest text-slate-900 mb-1.5" style={{ fontFamily: "'Helvetica Neue', Arial, sans-serif", letterSpacing: '0.18em' }}>Education</h2>
                  <div className="border-b border-slate-300 mb-3"></div>
                  <div className="space-y-3">
                    {education.map(edu => (
                      <div key={edu.id} className="flex justify-between items-baseline">
                        <div>
                          <h4 className="font-bold text-[0.88em] text-slate-900" style={{ fontFamily: 'inherit' }}>{edu.degree}</h4>
                          <p className="text-[0.8em] italic text-slate-600" style={{ fontFamily: 'inherit' }}>{edu.institution}</p>
                        </div>
                        <span className="text-[0.74em] text-slate-500 shrink-0 ml-2" style={{ fontFamily: "'Helvetica Neue', Arial, sans-serif" }}>{edu.endDate}</span>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {skills.length > 0 && (
                <div className={sectionWrapperStyle('skills')} onDragOver={(e) => handleDragOver(e, 'skills')} onDrop={(e) => handleDrop(e, 'skills')} onClick={() => onSectionClick?.('skills')}>
                  <DragHandle id="skills" />
                  <h2 className="text-[0.78em] font-black uppercase tracking-widest text-slate-900 mb-1.5" style={{ fontFamily: "'Helvetica Neue', Arial, sans-serif", letterSpacing: '0.18em' }}>Skills</h2>
                  <div className="border-b border-slate-300 mb-2"></div>
                  <p className="text-[0.8em] text-slate-700" style={{ fontFamily: 'inherit' }}>
              ...
              {projects.length > 0 && (
                <div className={sectionWrapperStyle('projects')} onDragOver={(e) => handleDragOver(e, 'projects')} onDrop={(e) => handleDrop(e, 'projects')} onClick={() => onSectionClick?.('projects')}>
                  <DragHandle id="projects" />
                  <h2 className="text-[0.78em] font-black uppercase tracking-widest text-slate-900 mb-1.5" style={{ fontFamily: "'Helvetica Neue', Arial, sans-serif", letterSpacing: '0.18em' }}>Projects</h2>
                  <div className="border-b border-slate-300 mb-3"></div>
                  <div className="space-y-3">
                    {projects.map(proj => (
                      <div key={proj.id}>
                        <div className="flex justify-between items-baseline">
                          <h3 className="font-bold text-[0.88em] text-slate-900" style={{ fontFamily: 'inherit' }}>{proj.name}</h3>
                          {proj.url && <span className="text-[0.72em] text-slate-400 shrink-0 ml-2" style={{ fontFamily: "'Helvetica Neue', Arial, sans-serif" }}>{proj.url.replace(/^https?:\/\//, '')}</span>}
                        </div>
                        {proj.technologies && <p className="text-[0.8em] italic text-slate-500" style={{ fontFamily: 'inherit' }}>{proj.technologies}</p>}
                        {proj.description && <p className="text-[0.8em] text-slate-700 leading-relaxed" style={{ fontFamily: 'inherit' }}>{proj.description}</p>}
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {(languages.length > 0 || courses.length > 0) && (
                <div className={sectionWrapperStyle('extras')} onDragOver={(e) => handleDragOver(e, 'extras')} onDrop={(e) => handleDrop(e, 'extras')} onClick={() => onSectionClick?.('extras')}>
                  ...
                  {languages.length > 0 && (
                    <>
                      <h2 className="text-[0.78em] font-black uppercase tracking-widest text-slate-900 mb-1.5" style={{ fontFamily: "'Helvetica Neue', Arial, sans-serif", letterSpacing: '0.18em' }}>Languages</h2>
                      <div className="border-b border-slate-300 mb-2"></div>
                      <p className="text-[0.8em] text-slate-700 mb-3" style={{ fontFamily: 'inherit' }}>
                  ...
                  {courses.length > 0 && (
                    <>
                      <h2 className="text-[0.78em] font-black uppercase tracking-widest text-slate-900 mb-1.5 mt-2" style={{ fontFamily: "'Helvetica Neue', Arial, sans-serif", letterSpacing: '0.18em' }}>Certifications & Courses</h2>
                      <div className="border-b border-slate-300 mb-2"></div>
                      <div className="space-y-1">
                        {courses.map(c => (
                          <div key={c.id} className="flex justify-between text-[0.8em]">
                            <span className="font-semibold text-slate-800" style={{ fontFamily: 'inherit' }}>{c.name}</span>
                            <span className="text-slate-500 shrink-0 ml-2" style={{ fontFamily: "'Helvetica Neue', Arial, sans-serif" }}>{c.institution} · {c.year}</span>
                          </div>
                        ))}
                      </div>
                    </>
