...
const ResumePreview: React.FC<Props> = ({ data, template, onSectionClick, onReorder, fontSize = 12 }) => {
  ...
  const ContactItem = ({ icon, text, dark = false }: { icon: string, text: string, dark?: boolean }) => {
    if (!text) return null;
    return (
      <div className={`flex items-center gap-2 text-[0.85em] font-medium ${dark ? 'text-slate-700' : 'text-white/80'}`}>
        <i className={`fas ${icon} w-3.5 text-center ${dark ? 'text-blue-600' : 'text-[#d4af37]'}`}></i>
      ...
      case 'summary':
        ...
        return (
          <div key={id} className={sectionWrapperStyle(id)} onDragOver={(e) => handleDragOver(e, id)} onDrop={(e) => handleDrop(e, id)} onClick={() => onSectionClick?.('summary')}>
            <DragHandle id={id} />
            <h2 className={`font-bold uppercase tracking-widest mb-2 ${isModernBlue ? 'text-blue-800 border-b-2 border-blue-50' : isExecutiveRed ? 'text-[#800000] border-b border-red-100 pb-1' : ''} text-[1em]`}>Perfil</h2>
            <p className="text-[0.9em] text-slate-600 leading-relaxed text-justify">{summary}</p>
          </div>
        );
      case 'experience':
        ...
        return (
          <div key={id} className={sectionWrapperStyle(id)} onDragOver={(e) => handleDragOver(e, id)} onDrop={(e) => handleDrop(e, id)} onClick={() => onSectionClick?.('experience')}>
            <DragHandle id={id} />
            <h2 className={`font-bold uppercase tracking-widest mb-4 ${isModernBlue ? 'text-blue-800 border-b-2 border-blue-50' : isExecutiveRed ? 'text-[#800000] border-b border-red-100 pb-1' : ''} text-[1em]`}>Experiência</h2>
            <div className="space-y-6">
              {experiences.map(exp => (
                <div key={exp.id}>
                  <div className="flex justify-between font-bold text-slate-900 text-[0.95em]">
                    <h3>{exp.position}</h3>
                    <span className="text-[0.8em] text-slate-400 font-normal">{exp.startDate} - {exp.endDate}</span>
                  </div>
                  <p className={`text-[0.85em] font-bold ${isExecutiveRed ? 'text-[#800000]' : 'text-blue-600'}`}>{exp.company}</p>
                  <p className="text-[0.85em] text-slate-500 mt-2 leading-relaxed whitespace-pre-wrap">{exp.description}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      case 'education':
        ...
        return (
          <div key={id} className={sectionWrapperStyle(id)} onDragOver={(e) => handleDragOver(e, id)} onDrop={(e) => handleDrop(e, id)} onClick={() => onSectionClick?.('education')}>
            <DragHandle id={id} />
            <h2 className={`font-bold uppercase tracking-widest mb-4 ${isModernBlue ? 'text-blue-800 border-b-2 border-blue-50' : isExecutiveRed ? 'text-[#800000] border-b border-red-100 pb-1' : ''} text-[1em]`}>Educação</h2>
            <div className="space-y-4">
              {education.map(edu => (
                <div key={edu.id}>
                  <h4 className="font-bold text-slate-900 text-[0.9em]">{edu.institution}</h4>
                  <p className="text-[0.85em] text-slate-500">{edu.degree}</p>
                  <p className="text-[0.75em] text-slate-300">{edu.startDate} - {edu.endDate}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      case 'skills':
        ...
        return (
          <div key={id} className={sectionWrapperStyle(id)} onDragOver={(e) => handleDragOver(e, id)} onDrop={(e) => handleDrop(e, id)} onClick={() => onSectionClick?.('skills')}>
            <DragHandle id={id} />
            <h2 className={`font-bold uppercase tracking-widest mb-4 ${isModernBlue ? 'text-blue-800 border-b-2 border-blue-50' : isExecutiveRed ? 'text-[#800000] border-b border-red-100 pb-1' : ''} text-[1em]`}>Habilidades</h2>
            <div className="flex flex-wrap gap-2">
              {skills.map(s => <span key={s.id} className="bg-slate-100 border border-slate-200 text-slate-700 px-2 py-1 rounded-md text-[0.8em] font-bold">{s.name}</span>)}
            </div>
          </div>
        );
      case 'extras':
        ...
        return (
          <div key={id} className={sectionWrapperStyle(id)} onDragOver={(e) => handleDragOver(e, id)} onDrop={(e) => handleDrop(e, id)} onClick={() => onSectionClick?.('extras')}>
            <DragHandle id={id} />
            <h2 className={`font-bold uppercase tracking-widest mb-4 ${isModernBlue ? 'text-blue-800 border-b-2 border-blue-50' : isExecutiveRed ? 'text-[#800000] border-b border-red-100 pb-1' : ''} text-[1em]`}>Idiomas & Cursos</h2>
            <div className="space-y-4">
              <div className="space-y-3">
                {languages.map(l => (
                  <div key={l.id} className="text-[0.85em]">
                    <div className="flex justify-between mb-1"><span>{l.name}</span><span className="opacity-60">{l.level}</span></div>
                    <div className="w-full h-1 bg-slate-100 rounded-full overflow-hidden">
                      <div className={`h-full ${isExecutiveRed ? 'bg-[#800000]' : 'bg-blue-400'}`} style={{width: `${l.percentage}%`}}></div>
                    </div>
                  </div>
                ))}
              </div>
              <div className="grid grid-cols-2 gap-3">
                {courses.map(c => (
                  <div key={c.id} className="text-[0.8em]">
                    <p className="font-bold text-slate-800 leading-tight">{c.name}</p>
                    <p className="text-slate-400 text-[0.9em]">{c.institution} • {c.year}</p>
                  </div>
                ))}
              </div>
            </div>
  ...
  const getTemplateLayout = () => {
    switch(template) {
          ...
          <div className="flex flex-row h-full">
            <div className="w-[75mm] bg-[#1e40af] text-white p-10 flex flex-col shrink-0">
              <div className="mb-10 text-center cursor-pointer" onClick={() => onSectionClick?.('info')}>
                <div className="w-36 h-36 rounded-2xl overflow-hidden mx-auto mb-6 border-4 border-white/20 shadow-xl bg-[#1e3a8a] flex items-center justify-center">
                  {personalInfo.photoUrl ? <img src={personalInfo.photoUrl} className="w-full h-full object-cover" /> : <i className="fas fa-user text-5xl opacity-20"></i>}
                </div>
                <h1 className="text-[1.8em] font-black uppercase tracking-tight leading-tight">{personalInfo.fullName || 'Seu Nome'}</h1>
                <p className="text-[0.9em] font-bold uppercase tracking-[0.1em] opacity-80 mt-2">{personalInfo.jobTitle}</p>
              </div>
              <div className="space-y-8">
                <section onClick={() => onSectionClick?.('info')}>
                  <h2 className="text-[0.85em] font-black uppercase tracking-widest opacity-50 mb-4">Contato</h2>
                  <div className="space-y-3">
          ...
          <div className="flex flex-col h-full bg-[#fcfcfc]">
            <header className="bg-[#800000] text-white p-12 flex justify-between items-center" onClick={() => onSectionClick?.('info')}>
              <div>
                ...
                <p className="text-[1em] font-medium uppercase tracking-[0.3em] text-red-200">{personalInfo.jobTitle || 'Seu Cargo'}</p>
              </div>
            ...
            <div className="p-12 flex-1 overflow-hidden">
               <div className="grid grid-cols-12 gap-12 h-full">
                  ...
                  <div className="col-span-4 space-y-10 border-l border-slate-100 pl-10">
                    {sectionOrder.map(id => (id !== 'summary' && id !== 'experience') && renderSectionContent(id))}
                  </div>
               </div>
            </div>
          ...
          <div className="flex flex-row h-full">
            <div className="w-[85mm] bg-[#0c1221] text-white p-12 flex flex-col shrink-0 border-r border-[#d4af37]/10">
              ...
              <div className="space-y-10">
                <section onClick={() => onSectionClick?.('info')}>
                  <h2 className="text-[0.8em] font-black uppercase tracking-[0.4em] text-[#d4af37]/70 mb-5 flex items-center gap-3"><span className="w-4 h-[1px] bg-[#d4af37]/40"></span> Contato</h2>
                  <div className="space-y-3">
                    <ContactItem icon="fa-envelope" text={personalInfo.email} />
                    <ContactItem icon="fa-phone" text={personalInfo.phone} />
                    <ContactItem icon="fa-map-marker-alt" text={personalInfo.location} />
          ...
          <div className="flex flex-row h-full">
            <div className="w-[70mm] bg-[#2D4F4F] text-white p-8 flex flex-col shrink-0">
              <div className="text-center mb-10" onClick={() => onSectionClick?.('info')}>
                ...
                <p className="text-[0.8em] uppercase opacity-75">{personalInfo.jobTitle}</p>
              </div>
          ...
          <div className="flex flex-col h-full bg-slate-50">
            <header className="bg-[#334155] text-white p-12 flex justify-between items-center" onClick={() => onSectionClick?.('info')}>
              <div>
                ...
                <p className="text-[1.1em] text-slate-300 font-light tracking-widest uppercase">{personalInfo.jobTitle}</p>
              </div>
            ...
            <div className="p-12 -mt-6 grid grid-cols-12 gap-8 flex-1 overflow-hidden">
              ...
              <div className="col-span-4 bg-white/50 p-8 rounded shadow-sm space-y-10">
                {sectionOrder.map(id => (id !== 'summary' && id !== 'experience') && renderSectionContent(id))}
              </div>
            </div>
          </div>
          ...
          <div className="p-16 font-sans flex flex-col h-full bg-white">
            <header className="grid grid-cols-12 gap-8 mb-16" onClick={() => onSectionClick?.('info')}>
              <div className="col-span-8">
                <h1 className="text-[3.8em] font-black tracking-tighter leading-none mb-4 uppercase">{personalInfo.fullName || 'Seu Nome'}</h1>
                <p className="text-[1.4em] font-bold text-blue-600 uppercase tracking-[0.2em]">{personalInfo.jobTitle}</p>
              </div>
              <div className="col-span-4 flex flex-col justify-end text-right space-y-1 text-[0.8em] font-bold uppercase text-slate-400">
          ...
          <div className="flex flex-row h-full bg-white">
            ...
            <div className="flex-1 p-16 flex flex-col h-full overflow-hidden">
              <header className="mb-16" onClick={() => onSectionClick?.('info')}>
                <h1 className="text-[3.5em] font-thin tracking-tight text-slate-900 mb-2">{personalInfo.fullName || 'Nome'}</h1>
                <p className="text-[1.2em] font-medium text-[#D32F2F] tracking-widest uppercase">{personalInfo.jobTitle}</p>
              </header>
      ...
      case 'modern_vitae':
        return (
          <div className="p-14 bg-[#fcfdfd] text-slate-800 flex flex-col h-full">
            <header className="flex items-center justify-between mb-12 border-b border-slate-100 pb-10" onClick={() => onSectionClick?.('info')}>
              <div className="max-w-[70%]">
                <h1 className="text-[2.2em] font-extrabold text-slate-900 leading-tight mb-2">{personalInfo.fullName || 'Nome'}</h1>
                <p className="text-[1.1em] font-bold text-blue-600 uppercase tracking-widest">{personalInfo.jobTitle}</p>
                <div className="flex flex-wrap gap-4 mt-4 text-[0.8em] text-slate-400">
        ...
        return (
          <div className="font-serif p-16 text-center h-full bg-white flex flex-col overflow-hidden">
            <header className="mb-12 border-b-4 border-double border-slate-900 pb-8" onClick={() => onSectionClick?.('info')}>
              <h1 className="text-[2.4em] font-bold uppercase tracking-widest mb-2">{personalInfo.fullName || 'Nome Completo'}</h1>
              <p className="text-[1em] italic text-slate-500">{personalInfo.jobTitle}</p>
              <div className="mt-4 flex justify-center gap-6 text-[0.8em] uppercase font-sans font-bold text-slate-600">
                <span>{personalInfo.email}</span>
                <span>{personalInfo.phone}</span>
              </div>
            </header>
