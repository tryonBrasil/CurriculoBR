...
export default function App() {
  ...
  // CORREÇÃO DO ERRO TS2367: activeTab agora é dinâmico e baseado nos STEPS
  ...
  if (view === 'home') {
    return (
      <div className="min-h-screen bg-white dark:bg-slate-900 flex flex-col items-center justify-center p-6 text-center">
        ...
        <button onClick={() => setView('templates')} className="bg-blue-600 text-white px-12 py-4 rounded-full font-bold uppercase tracking-widest shadow-2xl hover:bg-blue-700 transition-all">
          Começar
        </button>
      </div>
    );
    ...
    <div className="h-screen flex flex-col bg-white dark:bg-slate-950 overflow-hidden transition-colors duration-300">
      ...
      <nav className="no-print h-16 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between px-8 bg-white dark:bg-slate-900 z-50">
        ...
        <div className="flex gap-4">
          ...
          <button onClick={() => exportToDocx(data)} className="bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 px-6 py-2 rounded-full text-xs font-bold uppercase">Word</button>
          <button onClick={handlePrint} className="bg-blue-600 text-white px-6 py-2 rounded-full text-xs font-bold uppercase shadow-lg">PDF</button>
        </div>
      </nav>
      ...
      <div className="flex-1 flex overflow-hidden relative">
              ...
              <div className="space-y-6">
                {activeTab === 'info' && (
                  <div className="space-y-4">
                    ...
                    <Input label="Localização" value={data.location} onChange={(val) => updateData({...data, location: val})} />
                  </div>
                )}

                {activeTab === 'summary' && (
                  <div className="space-y-4">
                    <Input label="Resumo Profissional" value={data.summary} onChange={(val) => updateData({...data, summary: val})} isTextArea />
                  </div>
                )}
        ...
        {/* Lado do Preview */}
        <div ref={previewContainerRef} className={`flex-1 bg-slate-100 dark:bg-slate-950 overflow-hidden relative transition-all duration-300 ${mobileView === 'preview' ? 'flex' : 'hidden md:flex'} items-center justify-center p-4`}>
          <div className="origin-top transition-transform duration-300 shadow-2xl bg-white" style={{ transform: `scale(${previewScale})` }}>
      ...
      <div className="md:hidden fixed bottom-6 left-1/2 -translate-x-1/2 bg-white dark:bg-slate-800 shadow-2xl rounded-full p-1 border border-slate-200 dark:border-slate-700 flex z-50">
        <button onClick={() => setMobileView('editor')} className={`px-6 py-2 rounded-full text-[10px] font-black uppercase transition-all ${mobileView === 'editor' ? 'bg-blue-600 text-white' : 'text-slate-400'}`}>Editar</button>
        <button onClick={() => setMobileView('preview')} className={`px-6 py-2 rounded-full text-[10px] font-black uppercase transition-all ${mobileView === 'preview' ? 'bg-blue-600 text-white' : 'text-slate-400'}`}>Ver PDF</button>
      </div>
    </div>
  );
}
