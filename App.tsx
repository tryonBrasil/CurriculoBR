...
export default function App() {
  ...
  if (view === 'home') {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-white dark:bg-slate-900">
        <h1 className="text-6xl font-black mb-8 dark:text-white italic text-center">
          Curriculo<span className="text-blue-600">BR</span>
        </h1>
        <button 
          onClick={() => setView('templates')} 
          className="bg-blue-600 text-white px-10 py-4 rounded-full font-bold uppercase shadow-xl hover:scale-105 transition-all"
        >
  ...
  // VIEW: SELEÇÃO DE TEMPLATES
  if (view === 'templates') {
    return (
      <div className="min-h-screen bg-slate-50 dark:bg-slate-900 p-8">
        <div className="max-w-6xl mx-auto">
          <button onClick={() => setView('home')} className="mb-8 text-slate-400 font-bold text-xs uppercase">← Voltar</button>
          <h2 className="text-3xl font-black mb-12 dark:text-white text-center uppercase italic">Escolha um <span className="text-blue-600">Modelo</span></h2>
    ...
    <div className="h-screen flex flex-col bg-white dark:bg-slate-950 overflow-hidden">
      <nav className="h-16 border-b flex items-center justify-between px-8 bg-white dark:bg-slate-900">
        ...
        <button 
          ...
          className="bg-blue-600 text-white px-6 py-2 rounded-lg text-xs font-bold uppercase"
        >
          Exportar Word
        </button>
      </nav>
      ...
      <div className="flex-1 flex overflow-hidden">
        <div className="flex-1 overflow-y-auto p-12">
          <div className="max-w-xl mx-auto">
            <h2 className="text-2xl font-black mb-8 uppercase dark:text-white text-blue-600 italic">
            ...
            <div className="space-y-4">
              ...
              {activeTab === 'summary' && (
                <textarea 
                  className="w-full p-4 rounded-xl border-2 dark:bg-slate-800 dark:text-white min-h-[200px] outline-none focus:border-blue-600"
                  value={data.summary || ''}
                  onChange={(e) => updateData({...data, summary: e.target.value})}
                  placeholder="Seu resumo profissional..."
                />
              )}
            </div>

            <div className="mt-8 flex justify-between border-t pt-8 dark:border-slate-800">
              <button 
                disabled={currentStep === 0} 
                onClick={() => setCurrentStep(s => s - 1)} 
                className="text-slate-400 font-bold uppercase text-xs disabled:opacity-30"
              >
                Anterior
              </button>
              <button 
                onClick={() => currentStep < STEPS.length - 1 ? setCurrentStep(s => s + 1) : window.print()} 
                className="bg-slate-900 dark:bg-white dark:text-slate-900 text-white px-8 py-3 rounded-xl font-bold uppercase text-xs"
              >
...
} // <--- ESTA CHAVE FECHA A FUNÇÃO APP E ESTAVA FALTANDO!
